function adjustStoreInventory(bookId, change, reason, notes) {
  ...
  if (
    !found.object.preorder &&
    found.object.status !== "Draft" &&
    found.object.status !== "Archived"
  ) {
    found.sheet
      .getRange(found.row, statusCol)
      .setValue(next > 0 ? "Published" : "Out of Stock");
  }
  ...
  logInventoryChange_(
    ...
    previous,
    next,
    storeText_(reason, 200) || "Admin adjustment",
    "",
    admin,
    notes,
  );
...
function logInventoryChange_(
  ...
  const sheet = getStoreSpreadsheet_().getSheetByName(
    STORE_CONFIG.SHEETS.INVENTORY,
  );
  if (!sheet) return;
  const detail = notes
    ? String(reason || "") + (reason ? " — " : "") + storeText_(notes, 1000)
    : reason;
  sheet.appendRow([
    storeNow_(),
    sku,
...
function getStoreInventorySummary() {
  ...
  return readStoreBooks_(false).map(function (book) {
    return {
      ...
      lowStock:
        book.status === "Published" && book.stock <= book.lowStockThreshold,
      status: book.status,
    };
  });
}
