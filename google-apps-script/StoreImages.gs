function uploadStoreBookImage(bookId, fileName, mimeType, base64Data) {
  ...
  if (bytes.length > STORE_CONFIG.MAX_IMAGE_BYTES)
    throw new Error("Image must be 6 MB or smaller.");

  const safeName = sanitizeStoreFileName_(
    fileName || found.object.sku + ".png",
  );
...
function removeStoreBookImage(bookId) {
  ...
  if (found.object.imageFileId) {
    try {
      DriveApp.getFileById(found.object.imageFileId).setTrashed(true);
    } catch (error) {}
  }
  found.sheet
    .getRange(found.row, STORE_BOOK_HEADERS.indexOf("Image File ID") + 1)
    .clearContent();
  found.sheet
    .getRange(found.row, STORE_BOOK_HEADERS.indexOf("Image URL") + 1)
    .clearContent();
  found.sheet
...
function sanitizeStoreFileName_(name) {
  const cleaned = String(name || "book-image")
    .replace(/[^a-z0-9._-]+/gi, "-")
    .replace(/-+/g, "-");
