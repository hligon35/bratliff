...
function readStoreBooks_(publishedOnly) {
  const sheet = getStoreSpreadsheet_().getSheetByName(STORE_CONFIG.SHEETS.BOOKS);
  if (!sheet || sheet.getLastRow() < 2) return [];
  const values = sheet.getRange(2, 1, sheet.getLastRow() - 1, STORE_BOOK_HEADERS.length).getValues();
  const books = values.map(bookRowToObject_).filter(function (book) {
    return book.bookId && (!publishedOnly || ['Published', 'Coming Soon', 'Out of Stock'].indexOf(book.status) !== -1);
  });
...
function saveStoreBook(payload) {
  ...
  const stock = Math.max(0, Math.floor(storeNumber_(payload.stock, existing ? existing.object.stock : 0)));
  let status = storeText_(payload.status, 40) || 'Draft';
  if (status === 'Published' && stock <= 0 && !storeBool_(payload.preorder)) status = 'Out of Stock';
  ...
  const row = [
    ...
    stock,
    Math.max(0, Math.floor(storeNumber_(payload.lowStockThreshold, STORE_CONFIG.LOW_STOCK_DEFAULT))),
  ...
  if (existing) sheet.getRange(existing.row, 1, 1, STORE_BOOK_HEADERS.length).setValues([row]);
  else sheet.appendRow(row);

  if (existing && stock !== Number(existing.object.stock || 0)) {
    logInventoryChange_(sku, title, stock - Number(existing.object.stock || 0), Number(existing.object.stock || 0), stock, 'Admin adjustment', '', admin);
  }
...
function publishStoreBook(bookId) {
  ...
  const status = Number(found.object.stock || 0) > 0 || found.object.preorder ? 'Published' : 'Out of Stock';
...
function archiveStoreBook(bookId) {
  ...
  const found = findStoreBook_(bookId);
  if (!found) throw new Error('Book not found.');
  found.sheet.getRange(found.row, STORE_BOOK_HEADERS.indexOf('Status') + 1).setValue('Archived');
...
function findStoreBook_(bookId) {
  ...
  for (let i = 0; i < values.length; i++) {
    if (String(values[i][0]) === String(bookId)) return { sheet: sheet, row: i + 2, values: values[i], object: bookRowToObject_(values[i]) };
  }
  return null;
}
...
function ensureUniqueSku_(sku, bookId) {
  readStoreBooks_(false).forEach(function (book) {
    if (book.sku === sku && book.bookId !== bookId) throw new Error('That SKU is already in use.');
  });
}

function bookRowToObject_(row) {
  return {
    bookId: row[0], sku: row[1], isbn: row[2], title: row[3], subtitle: row[4], author: row[5],
    synopsis: row[6], shortDescription: row[7], format: row[8], category: row[9],
    price: Number(row[10] || 0), comparePrice: Number(row[11] || 0), stock: Number(row[12] || 0),
    lowStockThreshold: Number(row[13] || STORE_CONFIG.LOW_STOCK_DEFAULT), imageFileId: row[14], imageUrl: row[15],
    featured: Boolean(row[16]), comingSoon: Boolean(row[17]), preorder: Boolean(row[18]), status: row[19] || 'Draft',
    publicationDate: row[20] || '', created: row[21] || '', updated: row[22] || ''
  };
}
